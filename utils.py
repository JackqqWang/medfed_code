...
def get_public_dataset(args,dst_name=None):
    ...
    if dst_name=='NCT_dataset':
        ...
        dataset = NCT_dataset(
            '',extra_matrix_VIT,extra_matrix_RN50,train=False
        )
    ...
    if dst_name=='cifar':
        ...
        logits_path=""
        extra_matrix_RN50=torch.load(logits_path)
        _, preprocess = clip.load('', "cuda")
        cifar100_test = CIFAR100(root=os.path.expanduser(""), train=False, transform=preprocess, download=True)
        ...
        data_list=[]
        # image, label, extra_vector_VIT, extra_vector_RN50
        for image, label, extra_vector_VIT,extra_vector_RN50 in custom_dataloader:
            data_list.append([image,label,extra_vector_VIT,extra_vector_RN50])
    if dst_name=='skinseg':
        dataset_path=''
